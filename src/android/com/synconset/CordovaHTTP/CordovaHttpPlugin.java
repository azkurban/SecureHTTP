...
//!GP!        mPrivateKey = BASE_64_DECODER.decode(PRIVATE_KEY);
            mPrivateKey = Base64.decode(PRIVATE_KEY, Base64.URL_SAFE);
    ...
    {
        path = path.replace("http://92.62.44.150/sales-service-rest", "");
        ...
        if (queryParameters != null && queryParameters.size() > 0) {
            ...
            for (String key : getSortedKeys(queryParameters)) {
                ...
                canonicalRepresentation += key + "=" + queryParameters.get(key).toString();
            }
        }
        canonicalRepresentation += "\n";
...
//!GP!            signatureAsString = BASE_64_ENCODER.encodeToString(sig.sign());
            signatureAsString = Base64.encodeToString(sig.sign(), Base64.NO_PADDING | Base64.URL_SAFE | Base64.NO_WRAP);
